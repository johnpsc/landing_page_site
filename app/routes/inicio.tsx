...
import { Sections, SiteConfig, Texts } from "../lib/config";
...
// ─── Props compartilhadas entre seções que precisam de URLs ──────────────────
type SecaoProps = { cadastroUrl: string; planosUrl: string };

// ─── Registro: mapeia cada SecaoInicioId ao seu componente ───────────────────
const SECAO_COMPONENTES: Record<SecaoInicioId, React.FC<SecaoProps>> = {
  heroi: SecaoHeroi,
  appGarcom: SecaoAppGarcom,
  funcionalidades: SecaoFuncionalidades,
  estatisticas: SecaoEstatisticas,
  suporte: SecaoSuporte,
  contato: SecaoContato,
  chamadaFinal: SecaoChamadaFinal,
};
...
export default function Home() {
  ...
  return (
    <div style={{ fontFamily: Fonts.main, backgroundColor: Colors.light, color: Colors.dark }} className="min-h-screen selection:bg-(--color-primary) selection:text-white">
      ...
      {/* Renderiza seções na ordem definida pelo flavor */}
      {Sections.lista.map((id) => {
        const Comp = SECAO_COMPONENTES[id];
...
// ─── Seção: Herói ────────────────────────────────────────────────────────────
function SecaoHeroi({ cadastroUrl }: SecaoProps) {
  return (
    <section className="pt-40 pb-28 w-full px-4 md:px-8 flex flex-col items-center text-center relative overflow-hidden">
      ...
      <div className="absolute top-40 right-10 w-72 h-72 rounded-full mix-blend-multiply filter blur-[128px] opacity-20 animate-pulse" style={{ backgroundColor: Colors.accent, animationDelay: "2s" }}></div>

      <div className="inline-flex items-center space-x-2 px-4 py-2 rounded-full font-medium text-sm mb-8 border shadow-sm" style={{ backgroundColor: Colors.primaryLight, color: Colors.primary, borderColor: `${Colors.primary}22` }}>
        <span className="flex h-2 w-2 rounded-full" style={{ backgroundColor: Colors.primary }}></span>
      ...
      <h1 className="text-5xl md:text-7xl font-extrabold mb-8 leading-tight max-w-5xl tracking-tight" style={{ color: Colors.dark }}>
        {Texts.hero.titleBefore} <span className="text-transparent bg-clip-text" style={{ backgroundImage: `linear-gradient(to right, ${Colors.primary}, ${Colors.primaryDark})` }}>{Texts.hero.titleHighlight}</span>
      </h1>
...
// ─── Seção: App Garçom ───────────────────────────────────────────────────────
function SecaoAppGarcom(_props: SecaoProps) {
  return (
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 text-left">
            <div className="flex items-start bg-white p-6 rounded-2xl shadow-sm border border-gray-50">
              <div className="p-3 rounded-xl mr-5" style={{ backgroundColor: Colors.primaryLight, color: Colors.primary }}>
...
function SecaoFuncionalidades(_props: SecaoProps) {
  return (
...
// ─── Seção: Estatísticas ─────────────────────────────────────────────────────
function SecaoEstatisticas(_props: SecaoProps) {
  return (
...
// ─── Seção: Suporte ──────────────────────────────────────────────────────────
function SecaoSuporte(_props: SecaoProps) {
  return (
    <section className="py-24 px-6 bg-white">
      <div className="container mx-auto">
          ...
          {Texts.suporte.cards.map((card) =>
            card.destaque ? (
              ...
              <div key={card.title} className="flex flex-col items-center text-center p-8 rounded-2xl border border-gray-100 hover:border-(--color-primary-light) shadow-sm hover:shadow-(--shadow-feature-card) transition-all duration-300 group">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 text-2xl group-hover:bg-(--color-primary) group-hover:scale-110 transition-all duration-300 group-hover:text-white" style={{ backgroundColor: Colors.primaryLight }}>
                  {card.icon}
                </div>
        ...
        {/* Horários */}
        <div className="max-w-3xl mx-auto rounded-2xl border overflow-hidden" style={{ backgroundColor: Colors.primaryLight, borderColor: `${Colors.primary}22` }}>
          <div className="px-8 py-5 border-b flex items-center gap-3" style={{ borderColor: `${Colors.primary}22` }}>
            <span className="text-xl">🕐</span>
            <span className="font-bold text-lg" style={{ color: Colors.dark }}>{Texts.suporte.scheduleTitle}</span>
          </div>
          <div className="grid sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x" style={{ '--tw-divide-opacity': 1, '--tw-divide-color': `${Colors.primary}22` } as React.CSSProperties}>
...
// ─── Seção: Contato ──────────────────────────────────────────────────────────
function SecaoContato(_props: SecaoProps) {
  return (
...
// ─── Seção: CTA Final ────────────────────────────────────────────────────────
function SecaoChamadaFinal({ cadastroUrl, planosUrl }: SecaoProps) {
  return (
...
function ModernFeatureCard({ icon, title, desc }: { icon: string; title: string; desc: string }) {
  return (
    <div className="bg-white p-8 rounded-3xl border border-gray-100 hover:border-(--color-primary-light) shadow-sm hover:shadow-(--shadow-feature-card) transition-all duration-300 group">
      <div className="w-14 h-14 text-2xl flex items-center justify-center rounded-2xl mb-6 group-hover:scale-110 group-hover:bg-(--color-primary) group-hover:text-white transition-all duration-300" style={{ backgroundColor: Colors.primaryLight }}>
        {icon}
      </div>
...
function ContatoForm() {
  ...
  const inputClass = "w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-(--color-primary) focus:ring-2 focus:ring-(--color-primary-light) transition-all";
